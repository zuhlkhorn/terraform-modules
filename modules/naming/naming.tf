...
resource "azurecaf_name" "short" {
  for_each = toset(local.resources_with_short_names)
...
resource "azurecaf_name" "short-alphanumeric" {
  for_each = toset(local.resources_with_alphanumeric_names)
